...
def eval():
    ...
    with tf.Session(config=EvalConfig.session_conf) as sess:
        ...
        mixed_wav, src1_wav, src2_wav, wavfiles = data.get_wavs(EvalConfig.SECONDS, EvalConfig.NUM_EVAL)
        print('shape of src1: {} shape of src2: {}'.format(src1_wav.shape, src2_wav.shape))
    

        mixed_spec = to_spectrogram(mixed_wav)
        mixed_mag = get_magnitude(mixed_spec)
        mixed_mag_batch, padded_mixed_mag = model.spec_to_batch(mixed_mag)
        mixed_phase = get_phase(mixed_spec)
        mixed_phase_batch, padded_mixed_phase = model.spec_to_batch(mixed_phase)

        assert (np.all(np.equal(model.batch_to_spec(mixed_mag_batch, EvalConfig.NUM_EVAL), padded_mixed_mag)))

        (pred_src1_mag, pred_src2_mag) = sess.run(model(), feed_dict={model.x_mixed_mag: mixed_mag_batch, model.x_mixed_phase: mixed_phase_batch})
        ...
        print('shape of pred:{}'.format(pred_src1_wav.shape))
        ...
        if EvalConfig.EVAL_METRIC:
            # Compute BSS metrics
            print('shape of pred_src1_wav:{} shape of pred_src2_wav:{}'.format(pred_src1_wav.shape, pred_src2_wav.shape))
            print('shape of src1_wav:{} shape of src2_wav:{}'.format(src1_wav.shape, src2_wav.shape))
            if EvalConfig.BSS_EVAL:
                gnsdr, gsir, gsar = bss_eval_global(mixed_wav, src1_wav, src2_wav, pred_src1_wav, pred_src2_wav)
                print('gnsdr:{}\tgsir:{}\tgsar:{}'.format(gnsdr, gsir, gsar))
            if EvalConfig.PESQ:
                gpesq1, gpesq2 = pesq_global(src1_wav, src2_wav, pred_src1_wav, pred_src2_wav)
...
def bss_eval_global(mixed_wav, src1_wav, src2_wav, pred_src1_wav, pred_src2_wav):
    ...
    for i in range(EvalConfig.NUM_EVAL):
        ...
        print('number of src:{}-sdr:{}-sir:{}-sar:{}'.format(i,sdr,sir,sar))
        nsdr = sdr - sdr_mixed
...
def pesq_global(src1_wav, src2_wav, pred_src1_wav, pred_src2_wav):
    len_cropped = pred_src1_wav.shape[-1]
    src1_wav = src1_wav[:, :len_cropped]
    src2_wav = src2_wav[:, :len_cropped]
    gpesq_1 = np.zeros(1)
    ...
    total_len = 0
    for i in range(EvalConfig.NUM_EVAL):
        pesq_score_1 = pesq(src1_wav[i], pred_src1_wav[i], ModelConfig.SR)
        pesq_score_2 = pesq(src2_wav[i], pred_src2_wav[i], ModelConfig.SR)
        gpesq_1 += len_cropped * pesq_score_1
        gpesq_2 += len_cropped * pesq_score_2
        total_len += len_cropped
        print('number of test:{} pesq_1 score:{} pesq_2 score:{}'.format(i, pesq_score_1, pesq_score_2))
    gpesq_1 = gpesq_1 / total_len
    gpesq_2 = gpesq_2 / total_len
    print('gpesq_1:{} gpesq_2:{}'.format(gpesq_1, gpesq_2))
    return gpesq_1, gpesq_2
